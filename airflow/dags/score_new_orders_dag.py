...
with DAG(
    dag_id="Score_New_Orders",
    description="[ML] Score new DW records and write alerts",
    default_args=default_args,
    schedule_interval="0 3 * * *",
    catchup=False,
    tags=["ml", "scoring", "spark", "delta"],
) as dag:
    start_task = EmptyOperator(task_id="start")
    end_task = EmptyOperator(task_id="end")

    score_new_orders = SparkSubmitOperator(
        task_id="score_new_orders",
        conn_id="spark_default",
        application="/opt/airflow/etl/ml_features/score_new_orders.py",
        name="spark_score_new_orders",
        executor_cores=2,
        num_executors=2,
        executor_memory="2g",
        driver_memory="512m",
        ...
        application_args=[
            "--model",
            "s3a://lakehouse/models/olist_logreg",
            "--threshold",
            "0.55",
        ],
    )

    start_task >> score_new_orders >> end_task
